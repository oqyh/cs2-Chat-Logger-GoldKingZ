...
public class MySqlDataManager
{
    ...
    private const string InsertQuery = @"
        ...
        (date, map_name, steam_id, player_name, `where`, message)
        VALUES
        (@date, @map_name, @steam_id, @player_name, @where, @message);";
    ...
    private static async Task CopyDataToTempTable(MySqlConnection connection)
    {
        var copyQuery = $@"INSERT INTO chat_logs_new 
                        (date, map_name, steam_id, player_name, `where`, message)
                        SELECT date, map_name, steam_id, player_name, `where`, message 
                        FROM chat_logs 
                        WHERE date >= DATE_SUB(NOW(), INTERVAL @days DAY)
                        ORDER BY date;";
    ...
    public static async Task InsertChatLog(DateTime date, string mapName, string steamId, 
                                        string playerName, int where, string message)
    {
        using var connection = new MySqlConnection(ConnectionString);
        try
        {
    ...
    public static async Task BatchInsertMessages(List<Globals.ChatMessageStorage> messages)
    {
        ...
        try
        {
            ...
            queryBuilder.AppendLine("INSERT INTO chat_logs (date, map_name, steam_id, player_name, `where`, message) VALUES ");
            
            for (int i = 0; i < messages.Count; i++)
            {
                if (i > 0) queryBuilder.Append(", ");
                queryBuilder.Append($"(@date{i}, @map_name{i}, @steam_id{i}, @player_name{i}, @where{i}, @message{i})");
            }
            ...
            for (int i = 0; i < messages.Count; i++)
            {
                ...
                cmd.Parameters.Add($"@date{i}", MySqlDbType.DateTime).Value = DateTime.Now;
